...
export function QuickAddCard() {
  ...
  const handleSubmit = () => {
    ...
    if (!rawText) {
      toast.error("Введіть текст або скористайтесь мікрофоном.");
      return;
    }
    ...
    startTransition(async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(data?.error ?? "Помилка обробки запису.");
        }
        toast.success(`Контакт збережено: ${data.contact.fullName}`);
        setText("");
        router.refresh();
      } catch (err) {
      ...
      <div className="flex items-center justify-between pb-2">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">
            Введіть вільний текст або продиктуйте розмову для автоматичної структуризації.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 pt-1">
        <div className="relative flex items-stretch gap-2">
          <Textarea
            ...
            placeholder="Наприклад: 'Зустріч з Андрієм (VP Engineering у Grammarly). Знайомий з Олексієм з Petcube. Шукає ML ліда...'"
